...
- **Playwright MCP** returns a YAML accessibility snapshot with element references instead of pixels, with optional vision (coordinate) and testing capability groups (README, discovery scan). Its README positions a CLI plus skills as more token-efficient for coding agents and MCP for exploratory loops (unverified wording).
- **Chrome DevTools MCP** (v1.0.0 in May 2026 per the discovery scan) exposes performance trace start/stop and insight analysis for LCP, INP, and CLS, CrUX field data, accessibility-tree snapshots, screenshots, Lighthouse, and PWA install and launch.
...
- **Lighthouse 13.0** (October 2025, discovery scan) replaced many performance audits with the shared DevTools "insights"; 13.5.0 (2026-09-18) adds an agent-discovery group (llms.txt, WebMCP) per the discovery scan (unverified). The accessibility category is a weighted pass/fail average over axe-based audits, so a Lighthouse accessibility score is not a WCAG conformance statement and must not be reported as one (`prd.md` §13, no fabricated scores).

## 4. Design inputs

- **Figma MCP server** (formerly Dev Mode MCP server) is in beta per Figma's help and developer docs (discovery scan): design context, metadata, screenshots, variables, Code Connect, and design-system search; the remote server needs explicit frame links; write-to-canvas is free during beta and planned as a paid feature. Input for GAP-014.
- **Design tokens:** the Design Tokens Community Group published the Design Tokens Format Module 2025.10 as a Final Community Group Report dated 28 October 2025, which states it is not a W3C Standard (https://www.designtokens.org/TR/2025.10/format/, discovery scan). Style Dictionary v5 documents partial support. Candidate future anchor for OTH-DESIGN-SYSTEM token-consistency checks (tier T3 at most); not registered.
...
A recommendation that relies on a Limited feature must name a fallback. ARIA in HTML already covers the customizable-select elements (SRC-W3C-HTML-ARIA).

## 6. Native mobile test tooling

- **Apple XCUITest** `performAccessibilityAudit` (iOS 17+, macOS 14+) audit types include contrast, element detection, hit region, element description, Dynamic Type, clipped text, traits, and actions (Apple docs, discovery scan).
- **Google Accessibility Test Framework for Android** appears dormant (last artifact 2024, discovery scan). Device automation remains a later phase (`docs/audit-methodology.md` §2).
...
- **Toss Heuribot (휴리봇)** (SRC-TOSS-TECH-BLOG, articles of 2 December 2024): an LLM chatbot prompted to answer like a typical Toss user about an uploaded screen. Lessons that match this project's design: role and context framing instead of rules, OCR before reasoning, deliberately skimming fine print (compare `prd.md` §7.4 attention weights), and positioning as a quick check rather than validation. No accuracy data was published, so it is a design precedent, not evidence.
- **UICrit** (https://github.com/google-research-datasets/uicrit, UIST 2024, CC BY 4.0 per the discovery scan): 11,344 designer critiques with bounding boxes over 1,000 mobile UIs; candidate benchmark material for Phase 7 (GAP-021) and for calibrating UX_RISK wording.
...
- **Implication:** published LLM persona tools report designer satisfaction or task metrics, not agreement with real participants. This supports the existing safeguards (simulated labels, counts not percentages, no VIOLATION from personas; `docs/audit-methodology.md` §4) and keeps GAP-022 (evaluation metrics for synthetic users) open.
